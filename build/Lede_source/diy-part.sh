...
cat >$NETIP <<-EOF
uci set network.lan.ipaddr='10.10.10.10'                                    # IPv4 地址(openwrt后台地址)
...
sed -i "s/OpenWrt /Deng compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ           # 增加个性名字 ${Author} 默认为你的github帐号
